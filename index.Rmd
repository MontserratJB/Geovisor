...
is_github_pages <- grepl("github.io", Sys.getenv("SERVER_NAME", ""))
```

```{=html}
<!-- CSS mejorado para GitHub Pages -->
<style type="text/css">
/* Ocultar completamente todos los elementos del navbar */
...
/* Estilo para el encabezado personalizado */
.custom-header {
  width: 100%;
  text-align: center;
...
<!-- Encabezado personalizado con verificación de URL -->
<div class="custom-header">
  <img src="https://raw.githubusercontent.com/MontserratJB/Geovisor/main/encabezado.png" 
       alt="Encabezado del proyecto"
       onerror="this.style.display='none'; console.log('Error cargando imagen del encabezado');">
</div>

<!-- Script para forzar la ocultación del navbar después de que la página cargue -->
<script>
document.addEventListener('DOMContentLoaded', function() {
  // Ocultar navbar después de que la página cargue
  setTimeout(function() {
    var navbars = document.querySelectorAll('.navbar, .navbar-inverse, .navbar-default');
    ...
    var img = document.querySelector('.custom-header img');
    if (img) {
      img.onload = function() {
        console.log('Imagen del encabezado cargada correctamente');
      };
      img.onerror = function() {
        console.log('Error al cargar la imagen del encabezado');
        // Mostrar un encabezado de texto alternativo
        var headerDiv = document.querySelector('.custom-header');
        headerDiv.innerHTML = '<h1 style="background: linear-gradient(90deg, #1e3a8a, #3b82f6); color: white; padding: 20px; margin: 0; text-align: center;">Proyecto de amojonamiento, Datos GNSS</h1>';
      };
    }
  }, 100);
});
